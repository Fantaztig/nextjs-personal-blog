        ...
        {allBlogs.length >= 1 &&
          allBlogs.map(post => (
            <Link key={post.slug} href={`/blog/[slug]`} as={`/blog/${post.slug}`}>
              <a>
                <li className="flex flex-row rounded-l-full overflow-hidden bg-white m-auto" style={{borderTopRightRadius: "2500px", borderBottomRightRadius: "2500px"}}>
                    {
                  ...
                  <div className="flex flex-col justify-center pl-4 pr-16">
                    ...
                    <p className="mb-4">
                      <ReactMarkdown
                        source={post.frontmatter.summary}
                      />
                    </p>
